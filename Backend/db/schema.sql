...
-- =========================
-- 1:1 Scenario Detail Tables
-- Each uses scenario_id as PK + FK (enforces exactly one row max)
-- =========================
...
CREATE TABLE IF NOT EXISTS affordability_result (
  ...
  -- Optional: uncomment if you want strict allowed statuses
  -- CHECK (status IN ('OK', 'CAUTION', 'HIGH_RISK'))
);
...
-- =========================
-- 1:Many Term comparison results
-- =========================
...
-- Tracks views of tips for scenarios.
-- Your ERD didn’t define a PK; to allow multiple views over time,
-- we make (scenario_id, tip_id, viewed_at) the primary key.
